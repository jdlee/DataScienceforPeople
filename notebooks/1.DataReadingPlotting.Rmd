...
error.df = wine.df %>%
  chain_start %>%
  assert(in_set(1, 2, 3, 4, 5, 6, 7, 8), quality) %>% # Wine quality ratings must be between 1 and 8
...
ggplot(wine.df, aes(x = .panel_x, y = .panel_y)) +
  geom_point(size = .1, position = 'auto') + #1
  geom_autodensity() + #2
...
ggplot(wine_checked.df, aes(x = .panel_x, y = .panel_y)) +
  geom_point(aes(color = error), size = .1, position = 'auto') + #1
  geom_autodensity() + #2
  geom_density2d(colour = "grey70") + #3
  facet_matrix(vars(fixed.acidity:quality), 
               layer.diag = 2, layer.upper = 3, 
...
scaled.long.wine.df = long.wine.df %>% group_by(variables) %>% mutate(s.values = scale(values))
...
Data for most popular baby names is contained in csv files, one per year for the last 140 years.
...
ggplot(names.df, 
       aes(yob, percent, group = interaction(name, gender))) +
